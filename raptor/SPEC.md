...
### 1.1 Dual parse (gcre + Go Pratt)

Raptor does **not** select a parser with an environment variable.

Every source string is offered to both front ends:

1. **gcre** (`../gcre`, Grammar Compatible Regular Expressions) loads `runtime/raptor.raku`. That file is a **Pigeon-compatible Raku subset** (PEG ordered choice, no action blocks, no LTM). `gcre/raku.peg` is generated by Pigeon; authors still write `.raku`. Organic hatch: a subrule `<HOST_name>` is not a PEG rule — the Go host registers it (`gcre.RegisterHost`). Raptor’s grammar ends `TOP` with `<HOST_legacy_rest>?` so leftover input is parsed by the Pratt parser **from inside the grammar**, not via an environment variable.
2. **Go Pratt parser** (`runtime/lexer.go` + `runtime/parser.go`, `ParseProgramLegacy`) implements the full Raptor surface (LTM-like operator spelling, `infix:<+>`, heredocs, `{ * }` native stubs, statement forms gcre cannot consume).

`ParseProgram` runs both and keeps one tree:

- only gcre succeeds → gcre AST
- only Go succeeds → Go AST
- both succeed → Go Pratt AST (full language); gcre remains the PEG reading for subset-only programs (Go failed)
- both fail → combined error

Eval and the Moar compiler consume the same AST either way.
...
`raptor serve` hosts `web/` (tour + `raptor.wasm`). `raptor doc perlraptor` reads PodLit manuals in `docs/*.pod`.
